/* Park Empire v1.03.0 - Visual Life Update */
...
const DIFFICULTIES={
  easy:{
    label:"Easy",
    summary:"Relaxed economy: stronger demand, lower costs, lower incident risk and easier lending.",
  ...
  medium:{
    label:"Medium",
    summary:"Balanced park economics and business risk.",
  ...
  hard:{
    label:"Hard",
    summary:"Tough economy: weaker demand, higher costs, greater incident risk and stricter lending.",
...
function difficultyCfg(){
  return DIFFICULTIES[state?.difficulty||"medium"]||DIFFICULTIES.medium;
}

const RIDE_VISUALS={
  carousel:"carousel",dodgems:"dodgems",drop:"drop",rapids:"rapids",woodie:"coaster",launch:"launch",invert:"invert",dark:"dark",hyper:"hyper",giga:"giga"
};
const OUTLET_VISUALS={coffee:"coffee",burger:"burger",souvenir:"shop",icecream:"icecream",restaurant:"restaurant",premium:"premium"};

function rideVisual(id,mini=false){
  const kind=RIDE_VISUALS[id]||"coaster";
  const cls=`ride-visual ${kind} ${mini?"mini":""}`;
  if(kind==="carousel")return `<div class="${cls}" aria-hidden="true"><div class="rv-sky"></div><div class="carousel-roof"></div><div class="carousel-pole"></div><div class="carousel-deck"></div><i></i><i></i><i></i></div>`;
  if(kind==="dodgems")return `<div class="${cls}" aria-hidden="true"><div class="dodgem-roof"></div><div class="dodgem-floor"><i></i><i></i><i></i></div></div>`;
  if(kind==="drop")return `<div class="${cls}" aria-hidden="true"><div class="drop-tower"></div><div class="drop-car"></div><div class="drop-base"></div></div>`;
  if(kind==="rapids")return `<div class="${cls}" aria-hidden="true"><div class="water"></div><div class="rapid-rock r1"></div><div class="rapid-rock r2"></div><div class="raft"><i></i><i></i><i></i></div></div>`;
  if(kind==="dark")return `<div class="${cls}" aria-hidden="true"><div class="manor"><i></i><i></i><i></i><b></b></div><div class="moon"></div></div>`;
  return `<div class="${cls}" aria-hidden="true"><svg viewBox="0 0 320 120" preserveAspectRatio="none"><path class="track" d="M0 98 C42 98 45 34 82 34 C116 34 110 94 146 94 C184 94 172 18 215 18 C252 18 244 88 320 88"/><path class="supports" d="M42 98V65 M82 98V34 M146 98V94 M190 98V45 M215 98V18 M267 98V72"/></svg><div class="coaster-train"><i></i><i></i><i></i></div></div>`;
}

function outletVisual(id,mini=false){
  const kind=OUTLET_VISUALS[id]||"shop";
  return `<div class="outlet-visual ${kind} ${mini?"mini":""}" aria-hidden="true"><div class="store"><div class="awning"></div><div class="sign">${kind==="coffee"?"COFFEE":kind==="burger"?"BURGERS":kind==="icecream"?"ICE CREAM":kind==="premium"?"DINING":kind==="restaurant"?"RESTAURANT":"GIFTS"}</div><div class="door"></div><div class="window"></div></div><div class="outlet-icon">${kind==="coffee"?"☕":kind==="burger"?"●":kind==="icecream"?"◆":kind==="shop"?"★":"✦"}</div></div>`;
}

function weatherIcon(name){
  const n=(name||"").toLowerCase();
  if(n.includes("rain"))return `<span class="weather-icon rain"><i></i><i></i><i></i></span>`;
  if(n.includes("sunny"))return `<span class="weather-icon sun"><i></i></span>`;
  if(n.includes("overcast"))return `<span class="weather-icon cloud"><i></i></span>`;
  return `<span class="weather-icon cool"><i></i></span>`;
}

function parkSkyline(){
  const operational=state.rides.filter(r=>r.daysLeft<=0).slice(-6);
  const building=state.rides.filter(r=>r.daysLeft>0).slice(-2);
  const rideBits=operational.length?operational.map((r,i)=>`<div class="skyline-asset s${i%6}">${rideVisual(r.id,true)}</div>`).join(""):`<div class="skyline-empty">Build attractions and your park skyline grows here.</div>`;
  const buildBits=building.map(r=>`<div class="skyline-build"><span></span><b>${r.daysLeft}d</b></div>`).join("");
  return `<div class="park-skyline-scene"><div class="sky-sun"></div><div class="sky-cloud c1"></div><div class="sky-cloud c2"></div><div class="skyline-assets">${rideBits}${buildBits}</div><div class="guest-stream"><i></i><i></i><i></i><i></i><i></i></div><div class="skyline-ground"></div></div>`;
}
...
function newState(difficulty="medium"){
  const cfg=DIFFICULTIES[difficulty]||DIFFICULTIES.medium;
  return{
    version:1010,difficulty,day:1,cash:cfg.startingCash,debt:0,negativeDays:0,parkOpen:false,
    ...
    marketing:[],
    activity:[
      {title:"Company founded",detail:`Started on ${cfg.label} difficulty with ${money(cfg.startingCash)} cash.`},
    ...
    history:[],bankrupt:false,speed:1,pendingLoanOffer:null,adminGuestBoost:0
  };
}

let loadedState=load();
let state=loadedState||newState("medium");
let needsDifficultySelection=!loadedState;
let timer=null;
...
function load(){
  try{
    ...
    if(s.version!==1010)return null;
    s.difficulty=s.difficulty||"medium";
    s.adminGuestBoost=s.adminGuestBoost||0;
    s.live=blankLive();
    s.parkOpen=false;
    if(typeof s.pendingLoanOffer==="undefined")s.pendingLoanOffer=null;
    return s;
  }catch{return null;}
}
...
function demandEstimate(weather=state.live.weather){
  ...
  return Math.max(0,Math.round((base*rep*rating*priceFactor()*(1+marketingBoost())*seasonFactor(state.day)*weekendFactor(state.day)*(weather?.demand||1)*difficultyCfg().demand)+(state.adminGuestBoost||0)));
}
...
function fixedCosts(){
  ...
  return (upkeep+outletFixed+payroll()+overhead())*difficultyCfg().fixedCosts;
}
...
function borrowingLimit(){
  ...
  return Math.max(startupFloor,Math.min(assetLimit+earningsLimit,parkValueGross()*.6))*difficultyCfg().loanAccess;
}
...
function forecast(){
  ...
  const variable=guests*1.55*difficultyCfg().variableCosts;
...
function startDay(){
  ...
  render();
if(needsDifficultySelection)openDifficultyModal("fresh");
}

function maybeBreakdown(){
  if(Math.random()>.035*difficultyCfg().incidentRisk)return;
  ...
  owned.down=true;
  const repair=Math.round(ride.cost*rand(.0025,.009)*difficultyCfg().incidentCost);
  const compensation=Math.round(state.live.guestsInPark*rand(.2,.7)*6*difficultyCfg().incidentCost);
...
function liveTick(){
  ...
  const tickCosts=fixedCosts()/L.totalTicks+outletCost+arrival*1.55*difficultyCfg().variableCosts+debtServiceDaily()/L.totalTicks;
...
function bankManagerResponse(requested){
  ...
  if(requested<10000){
    return{
      approved:false,
      message:"The minimum commercial facility we can discuss is £10,000."
    };
  }
...
let difficultyModalMode="fresh";

function openDifficultyModal(mode="fresh"){
  difficultyModalMode=mode;
  selectedDifficulty="medium";
  document.querySelectorAll(".difficulty-choice").forEach(x=>x.classList.toggle("selected",x.dataset.difficulty==="medium"));
  document.getElementById("difficultyStartBtn").textContent="Start New Company - Medium";
  document.getElementById("difficultyCancelBtn").classList.toggle("hidden",mode==="fresh");
  document.getElementById("difficultyModal").classList.remove("hidden");
}

function startDifficultyGame(){
  if(timer){clearInterval(timer);timer=null;}
  state=newState(selectedDifficulty);
  needsDifficultySelection=false;
  localStorage.setItem(SAVE_KEY,JSON.stringify(state));
  document.getElementById("difficultyModal").classList.add("hidden");
  render();
  toast(`${DIFFICULTIES[selectedDifficulty].label} difficulty selected.`);
}

function resetGame(){
  if(!confirm("Start a new company? Your current save will be replaced after you choose a difficulty."))return;
  openDifficultyModal("reset");
}
...
function render(){
  ...
  document.getElementById("difficultyIndicator").textContent=difficultyCfg().label;
  document.getElementById("difficultySummary").textContent=difficultyCfg().summary;
  ...
  document.getElementById("weatherStat").innerHTML=`${weatherIcon(L.weather.name)}<span>${L.weather.name}</span>`;
  ...
  const skyline=document.getElementById("parkSkyline"); if(skyline) skyline.innerHTML=parkSkyline();
  ...
  if(!state.history.length){
    reportSummary.innerHTML=`
      ...
      <div><span>Total profit</span><strong>£0</strong></div>
    ...
    profitTrendValue.textContent="£0";
  }else{
...
document.getElementById("releaseCloseBtn").onclick=()=>{
  document.getElementById("releaseModal").classList.add("hidden");
};
document.getElementById("releaseCloseTop").onclick=()=>{
  document.getElementById("releaseModal").classList.add("hidden");
};
document.getElementById("releaseModal").addEventListener("click",e=>{
  if(e.target.id==="releaseModal")document.getElementById("releaseModal").classList.add("hidden");
});
...
document.querySelectorAll(".difficulty-choice").forEach(btn=>{
  btn.addEventListener("click",()=>{
    ...
    document.querySelectorAll(".difficulty-choice").forEach(x=>x.classList.toggle("selected",x===btn));
    document.getElementById("difficultyStartBtn").textContent=`Start New Company - ${DIFFICULTIES[selectedDifficulty].label}`;
  });
});
document.getElementById("difficultyStartBtn").onclick=startDifficultyGame;
document.getElementById("difficultyCancelBtn").onclick=()=>{
  document.getElementById("difficultyModal").classList.add("hidden");
};
