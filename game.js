/* Park Empire v1.05.0 - PixiJS WebGL World Integration */
...
const RIDE_VISUALS={
  carousel:{mark:"CR",family:"Family"},
  dodgems:{mark:"DG",family:"Family"},
  drop:{mark:"SD",family:"Thrill"},
  rapids:{mark:"RR",family:"Water"},
  woodie:{mark:"TR",family:"Coaster"},
  launch:{mark:"VL",family:"Coaster"},
  invert:{mark:"SB",family:"Coaster"},
  dark:{mark:"MM",family:"Dark Ride"},
  hyper:{mark:"TT",family:"Coaster"},
  giga:{mark:"A3",family:"Coaster"}
};

function rideVisual(id){
  const v=RIDE_VISUALS[id]||{mark:"RD",family:"Ride"};
  return `<div class="asset-thumb ride-thumb thumb-${id}">
    <div class="asset-thumb-mark">${v.mark}</div>
    <div><span>${v.family}</span><strong>LIVE PARK ASSET</strong></div>
  </div>`;
}

function outletVisual(id){
  const marks={coffee:"CF",burger:"BG",souvenir:"GS",icecream:"IC",restaurant:"FR",premium:"PD"};
  return `<div class="asset-thumb outlet-thumb thumb-${id}">
    <div class="asset-thumb-mark">${marks[id]||"SH"}</div>
    <div><span>Commercial Asset</span><strong>LIVE PARK ASSET</strong></div>
  </div>`;
}
...
function parkWorldSnapshot(){
  return{
    ...
    rating:state.rating,
    cash:state.cash,
    rides:state.rides.map(o=>{
      const r=rideById(o.id);
      const stats=latestRideStat(o.uid);
      return{
        uid:o.uid,
        id:o.id,
        name:r.name,
    ...
    outlets:state.outlets.map(o=>{
      const x=outletById(o.id);
      const stats=latestOutletStat(o.uid);
      return{
        uid:o.uid,
        id:o.id,
        name:x.name,
...
window.ParkEmpireWorldBridge={
  snapshot:parkWorldSnapshot
};
