/* Park Empire v1.04.0 - Professional 3D Attraction Engine */
...
function rideVisual(id,mini=false){
  const scene=RIDE_VISUALS[id]||"coaster";
  return `<div class="ride-visual-3d ${mini?"mini":""}">
    <canvas class="park3d-canvas" data-scene="${scene}" data-mini="${mini?1:0}" aria-label="Animated 3D attraction preview"></canvas>
    <div class="render-badge"><span></span> LIVE 3D</div>
  </div>`;
}

function outletVisual(id,mini=false){
  return `<div class="ride-visual-3d outlet ${mini?"mini":""}">
    <canvas class="park3d-canvas" data-scene="outlet" data-outlet="${id}" data-mini="${mini?1:0}" aria-label="Animated 3D outlet preview"></canvas>
    <div class="render-badge"><span></span> LIVE 3D</div>
  </div>`;
}
...
function weatherIcon(name){
  ...
  if(n.includes("rain"))return `<span class="weather-chip">RAIN</span>`;
  if(n.includes("sunny"))return `<span class="weather-chip">SUN</span>`;
  if(n.includes("overcast"))return `<span class="weather-chip">CLOUD</span>`;
  return `<span class="weather-chip">COOL</span>`;
}

function parkSkyline(){
  const operational=state.rides.filter(r=>r.daysLeft<=0).slice(-8).map(r=>r.id);
  const building=state.rides.filter(r=>r.daysLeft>0).slice(-3).map(r=>({id:r.id,days:r.daysLeft}));
  const payload=encodeURIComponent(JSON.stringify({rides:operational,building}));
  return `<div class="park-3d-overview">
    <canvas class="park3d-canvas park3d-overview-canvas" data-scene="park" data-park="${payload}" aria-label="Animated 3D park overview"></canvas>
    <div class="park-render-label"><span></span> LIVE PARK VISUALISATION</div>
  </div>`;
}
...
function render(){
  ...
  if(window.Park3D)window.Park3D.scan();
}
